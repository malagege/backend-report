...
public class ECache {
	...
	public synchronized static void put(String key, Object value) {
		ECache.put(key, value, 259200000L);
	...
	public synchronized static <T> List<T>  getList(){
		DATA_BASE.values().size();
		List<T> list = DATA_BASE.values().stream().map(m -> (T)m.value).collect(Collectors.toList());
		// List<T> listOfValues = new ArrayList<T>( (Collection<? extends T>) DATA_BASE.values());
		return list;
	}
}